...
function loadSettings(sender)
{
    ...
    reader.onload = function()
    {
        try 
        {
			...
			var valid = (configArray.length > 0) && (configArray[0].Type == "pgNodeCfg");
			if (!valid)
			{
...
function setDisplayOptions()
{
	...
	setVisibility(configWorkData.InterfaceIndex > 1, configBushbyBox);
}
...
function setProdType(sender)
{
	...
	if (configWorkData.HatTypeList[oldHatMode].Type > configWorkData.InterfaceTypeList[oldCommMode].Type) 
	{
...
function setUseALM(sender)
{
	...
	else
	{
		...
		if (elPos >= 0)
			configWorkData.ALMIndex.splice(ALMId,1);
	}
