...
function prepareFileSeq() //object specific function to create partial files
{
	...
	thisEntry.MQTT = JSON.parse(JSON.stringify(configWorkData.MQTT));
...
function setLEDBasics(sender)
{
	...
	if (sender.id == "mqtopledask")
		configWorkData.MQTT.Subscribe[1].Topic = sender.value;
	if (sender.id == "incladdrask")
		configWorkData.MQTT.Subscribe[1].InclAddr = sender.checked;
...
function constructPageContent(contentTab)
{
	...
	mainScrollBox = createEmptyDiv(contentTab, "div", "pagetopicboxscroll-y", "nodeconfigdiv");
		...
		ledDefTable = createDataTable(colorTableDiv, "tile-1", ["Pos","LED Selector", "LED Command Sequence Editor"], "ledconfig", "");
...
function loadDataFields(jsonData)
{
	...
	writeInputField("mqtopledset", jsonData.MQTT.Subscribe[0].Topic);
	writeCBInputField("incladdrset", jsonData.MQTT.Subscribe[0].InclAddr);
	writeInputField("mqtopledask", jsonData.MQTT.Subscribe[1].Topic);
	writeCBInputField("incladdrask", jsonData.MQTT.Subscribe[1].InclAddr);
	writeInputField("mqtopledreply", jsonData.MQTT.Publish[0].Topic);
	writeCBInputField("incladdrreply", jsonData.MQTT.Publish[0].InclAddr);
