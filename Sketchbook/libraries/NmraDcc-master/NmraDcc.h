...
//#define BOOSTER_MODE  //if defined, insert commands to measure value of analog pin
...
#define FLAGS_CV29_BITS		(FLAGS_OUTPUT_ADDRESS_MODE | FLAGS_DCC_ACCESSORY_DECODER)
  ...
  uint8_t measurePinA;
  uint8_t measurePinB;
  void setMeasurePins(uint8_t measurePinNumA, uint8_t measurePinNumB); //call this to activate Booster mode. Use the same pin numbers if only one booster channel
#endif
...
void pin( uint8_t ExtIntPinNum, uint8_t EnablePullup);
  ...
  static void NmraDccTaskA(void* pvParams)
  {
      while(true)
          ((NmraDcc*)pvParams)->TaskMeasureA();
  }
...
#ifdef ESP32
extern void reportCurrentA(uint16_t currVal) __attribute__ ((weak));
extern void reportCurrentB(uint16_t currVal) __attribute__ ((weak));
#endif
