...
DCC_PROCESSOR_STATE DccProcState ;
	...
	static TaskHandle_t xTaskToNotifyA = NULL;
	uint8_t boostSel = 0;
	uint8_t procSel = 0;
...
#endif
{
	...
	if (boosterMode)
	{
		...
		if (procSel & 0x01) //do this every other time, meaning once per bit in order to not run out of time in the preambel sequence
		{
			vTaskNotifyGiveFromISR(xTaskToNotifyA, &xHigherPriorityTaskToken);
			portYIELD_FROM_ISR( );
		}
		procSel++;
	}
...
/* I think this can be eliminated. It is only called for mobile decoders based on address selection
...
#ifdef ESP32
void NmraDcc::setMeasurePins(uint8_t measurePinNumA, uint8_t measurePinNumB) //call set MeasurePins to activate boosterMode
{
  measurePinA = measurePinNumA;
  measurePinB = measurePinNumB;
//  pinMode(4, OUTPUT);
//  pinMode(16, OUTPUT);
  xTaskCreate(NmraDcc::NmraDccTaskA, "NmraDccTaskA", 4096, (void* ) this, 2, &xTaskToNotifyA);
  boosterMode = true;
}
#endif
...
void NmraDcc::TaskMeasureA()
{
	if (boosterMode) 
	{
		...
		if(ulNotificationValue == 1)
		if (boostSel & 0x01)
		{ //total time for the following is about 40 micros
//			digitalWrite(4, !digitalRead(4));
			uint16_t thisMeasure = analogRead(measurePinA);
//			digitalWrite(4, !digitalRead(4));
			thisMeasure = analogRead(measurePinA);
//			digitalWrite(4, !digitalRead(4));
			thisMeasure += analogRead(measurePinA);
//			digitalWrite(4, !digitalRead(4));
			thisMeasure += analogRead(measurePinA);
			if (reportCurrentA)
				reportCurrentA(round(thisMeasure/3));
		}
		else
		{ //total time for the following is about 40 micros
//			digitalWrite(16, !digitalRead(16));
			uint16_t thisMeasure = analogRead(measurePinB);
//			digitalWrite(16, !digitalRead(16));
			thisMeasure = analogRead(measurePinB);
//			digitalWrite(16, !digitalRead(16));
			thisMeasure += analogRead(measurePinB);
//			digitalWrite(16, !digitalRead(16));
			thisMeasure += analogRead(measurePinB);
			if (reportCurrentB)
				reportCurrentB(round(thisMeasure/3));
		}
		boostSel++;
	}
	yield();
