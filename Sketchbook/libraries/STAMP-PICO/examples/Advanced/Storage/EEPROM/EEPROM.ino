...
* date：2021/9/25
*******************************************************************************
  The values stored in the EEPROM will remain in the EEPROM even after the STAMP-PICO is disconnected.
  When a new program is uploaded to the STAMP-PICO, the values stored in the EEPROM can still be called or modified by the new program.
  储存于EEPROM的数值即使在断开 STAMP-PICO电源后仍会保存在EEPROM中
  当新程序上传到 STAMP-PICO后，储存于EEPROM中的数值仍然可以被新的程序调用或者修改
*/
#include <Arduino.h>
#include <EEPROM.h>
