...
//				Serial.printf("Prog timeout");
        ...
        if ((millis() - lastThrottleRefresh) > throttleRefreshInterval)
        { 
            bool keepGoing = true;

            if (throttleSlotIndex == 0)
            {
                throttleSlotIndex = maxSlots;
                throttleRefreshIndex = 6;

                if (throttleRefreshPriority == 0)
                {
                    throttleRefreshPriority = 4;
                }

                else
                {
                    throttleRefreshPriority--;
                }
            }

            while ((throttleSlotIndex > 0) && keepGoing)
            {
                uint8_t slotStatus = slotBuffer[throttleSlotIndex][STAT];
                uint8_t speed = slotBuffer[throttleSlotIndex][SPD];
                uint32_t currFct = getLocoFctStatus(&slotBuffer[fctRefreshSlotIndex]);
                
                if ((slotStatus & 0x30) == slotCommon || (slotStatus & 0x30) == slotActive)
                {
                    if (speed > 2 || throttleRefreshPriority == 0)
                    {
                        while ((throttleRefreshIndex > 0) && keepGoing)
                        {
                            switch (throttleRefreshIndex)
                            {
                                case 1:
                                {
                                    prepSlotSpeedMsg(&txBuffer, throttleSlotIndex, speed);
                                    keepGoing = false;
                                }
                                break;
...
//                                        Serial.printf("Send Group %i Slot %i Data %2X\n", fctGroupIndex, fctRefreshSlotIndex, currFct);
                                ...
                                case 5:
                                {
                                    if ((currFct & 0x00FF0000) > 0 && fctRefresh) //13-20
                                    {
//                                        Serial.printf("Send Group %i Slot %i Data %2X\n", fctGroupIndex, fctRefreshSlotIndex, currFct);
                                        prepSlotExtFctMsg(&txBuffer, getAddrOfSlot(fctRefreshSlotIndex), 13, currFct);
                                        keepGoing = false;
                                    }
                                }
                                break;
                                ...
                                case 6:
                                {
                                    if ((currFct & 0xFF000000) > 0 && fctRefresh) //21-28
                                    {
//                                        Serial.printf("Send Group %i Slot %i Data %2X\n", fctGroupIndex, fctRefreshSlotIndex, currFct);
                                        prepSlotExtFctMsg(&txBuffer, getAddrOfSlot(fctRefreshSlotIndex), 21, currFct);
                                        keepGoing = false;
                                    }
                                }
                                break;
                    ...
                    throttleRefreshIndex--;
                }

                if (throttleRefreshIndex == 0)
                {
                    throttleSlotIndex--;
                    throttleRefreshIndex = 6;
                }
    ...
    if (fctRefresh)
        if ((millis() - lastFctRefresh) > fctRefreshIntv)
        {
            ...
            while ((fctRefreshSlotIndex > 0) && keepGoing)
            {
                ...
                {
                    if (currFct > 0)
                        Serial.printf("Testing Slot %i Group %i Data: %8X\n", fctRefreshSlotIndex, fctGroupIndex, currFct);
