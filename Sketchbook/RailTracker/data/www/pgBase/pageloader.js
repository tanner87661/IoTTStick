...
var targetSize = 8192; //16kb target length of file strings, should always fit into 16kB wsBuffer of asyncServer
...
function resetDistCounter()
{
//	console.log("Setting LED: {\"Cmd\":\"SetLED\", \"LedNr\":[" + ledNr + "]}");
	ws.send("{\"Cmd\":\"ResetDist\"}");
}

function resetIMUOfset()
{
	ws.send("{\"Cmd\":\"ResetIMU\"}");
}

function resetPosOfset()
{
	ws.send("{\"Cmd\":\"ResetPos\"}");
}

function startTracking(FileName)
{
	ws.send("{\"Cmd\":\"StartRecording\", \"FileName\":\"" + FileName + "\"}");
}

function stopTracking()
{
	ws.send("{\"Cmd\":\"EndRecording\"}");
}
...
//			console.log(JSON.stringify(myArr.Data));
  		...
  		if ((myArr.Cmd == "PosData") && (scriptList.Pages[currentPage].ID == "pgTrMonCfg"))
