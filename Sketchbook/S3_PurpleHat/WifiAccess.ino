...
//    wifiManager.resetSettings();
    ...
    {
      USBSerial.println("Set Static IP Mode");
    ...
    if (wifiCfgMode == 1)  //STA mode is used, so wifiManager can handle AP selection and password
    {
        ...
        //in seconds
        USBSerial.println("Set STA Mode");
        wifiManager.setTimeout(120); 
        String hlpStr = "New_IoTT-StampS3_" + String((uint32_t)ESP.getEfuseMac());
        if (!wifiManager.autoConnect(hlpStr.c_str()))
        {
          USBSerial.println("failed to connect and hit timeout, setting up AP, if configured, otherwise restart ESP32");
    ...
    if (wifiCfgMode == 2) //if AP is needed, define the AP settings
    {
      ...
      WiFi.setHostname(deviceName.c_str());
      USBSerial.print("Local Access Point at ");
  ...
  if (!wifiCancelled)
    if (!wifiAlwaysOn)
    {
      ...
      if (millis() > (lastWifiUse + shutTimeOut))
      {
        ...
        WiFi.mode(WIFI_OFF);
        delay(100); 
        USBSerial.println("Wifi disabled. Click button B to re-enable");
      }
    }
}
